import { AspectRatio, Box, Card, CardContent, Grid, List, ListItem, ListItemContent, ListItemDecorator, Stack, Typography, useTheme } from "@mui/joy"
		...
		<Grid container height='100vh' >
			{
				...
				<Grid container lg={6}
					md={6}
					alignContent='center' justifyContent='center' sx={{ backgroundColor: 'var(--joy-palette-background-body)', borderRight: '2px solid var(--joy-palette-primary-outlinedBorder)', padding: '20px' }} className="animate__animated animate__slideInRight">
					<div style={{ textAlign: 'center' }}>
						<Typography fontSize='36px' >Bienvenidx a</Typography><Typography ml={-1.5} fontSize='36px' color="primary" fontStyle='italic'>Papalote</Typography>
					</div>
					<Grid container spacing={3} sx={{ flexGrow: 1, alignItems: 'stretch' }} pt={1}>
						{
							phrases.map((item, index) => (
								<Grid item xs={12} sm={6} md={6} lg={4} key={index} sx={{ display: 'flex', flexDirection: 'column' }}>
									<Card sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'start' }}>
										<Box sx={{ borderRadius:100, backgroundColor: 'var(--joy-palette-neutral-outlinedBorder)' }} p={2} m='auto' width={90} height={90} >
											<AspectRatio ratio='1' color='none'>
												 
											{item.icon}
											</AspectRatio>
										</Box>
										<Typography level="title-md" fontStyle='italic'>{item.title}</Typography>
